...
class AlgebraicModel(ABC):
    ...
    def __rshift__(self, other: Union['AlgebraicModel', Callable]) -> 'AlgebraicModel':
        """Apply function or compose with model: model >> func or model >> model2"""
        if isinstance(other, AlgebraicModel):
            return ComposedModel([self, other])
        return FunctionModel(self, other)

    def __invert__(self) -> 'AlgebraicModel':
        """Invert probabilities: ~model (1 - p for each probability)"""
        return InvertedModel(self)

    # Power operations
    def __pow__(self, exponent: float) -> 'AlgebraicModel':
        """Temperature/sharpening: model ** exponent (temperature = 1/exponent)"""
        # The exponent is used to sharpen (>1) or smooth (<1) the distribution
        # Temperature is the inverse of the exponent
        temperature = 1.0 / exponent if exponent != 0 else 1.0
        return TemperatureModel(self, temperature)
...
class LongestSuffixTransform(ContextTransform):
    """Keep the last n tokens from context (longest suffix of length n)."""

    def __init__(self, n: int):
        """
        Initialize with suffix length.

        Args:
            n: Number of tokens to keep from the end of context
        """
        self.n = n

    def transform(self, context: List[str]) -> List[str]:
        """Return the last n tokens from context."""
        if not context:
            return []
        return context[-self.n:] if len(context) >= self.n else context
...
class RecencyWeightTransform(ContextTransform):
    """Apply recency weighting with exponential decay to context window."""

    def __init__(self, alpha: float = 0.9, window: Optional[int] = None):
        """
        Initialize recency weighting.

        Args:
            alpha: Decay factor for recency weighting
            window: Optional window size to limit context
        """
        self.alpha = alpha
        self.window = window

    def transform(self, context: List[str]) -> List[str]:
        """Apply recency weighting by filtering based on weighted importance."""
        if not context:
            return []

        # Apply window if specified
        windowed = context[-self.window:] if self.window else context

        # For simplicity, return the windowed context
        # (weights would be applied at prediction time in a real implementation)
        return windowed
...
class SemanticFilterTransform(ContextTransform):
    ...
    def __init__(self, threshold: float = 0.5, relevance_fn: Optional[Callable[[str], float]] = None):
        """
        Initialize semantic filter.

        Args:
            threshold: Minimum relevance score to keep a word
            relevance_fn: Optional function to compute relevance (default: simple heuristic)
        """
        self.threshold = threshold
        self.relevance_fn = relevance_fn or self._default_relevance

    def _default_relevance(self, word: str) -> float:
        """Simple default relevance based on word length and common patterns."""
        # Simple heuristic: longer words and non-stopwords are more relevant
        if len(word) <= 2:
            return 0.3
        elif len(word) >= 6:
            return 0.8
    ...
    def transform(self, context: List[str]) -> List[str]:
        """Filter words by semantic relevance."""
...
class ScaledModel(AlgebraicModel):
    ...
    def __init__(self, model: AlgebraicModel, scalar: float):
        self.model = model
        self.scale = scalar  # Use 'scale' as the attribute name for consistency

    def predict(self, context: List[str], top_k: int = 50) -> Dict[str, float]:
        predictions = self.model.predict(context, top_k)
        return {token: prob * self.scale for token, prob in predictions.items()}
...
class InvertedModel(AlgebraicModel):
    """Invert probability distribution: 1 - p for each token."""
    ...
    def predict(self, context: List[str], top_k: int = 50) -> Dict[str, float]:
        ...
        inverted = {token: 1.0 - prob for token, prob in predictions.items()}

        # Renormalize
        total = sum(inverted.values())
        if total > 0:
            inverted = {k: v/total for k, v in inverted.items()}

        sorted_preds = sorted(inverted.items(), key=lambda x: x[1], reverse=True)
        return dict(sorted_preds[:top_k])
...
class FunctionModel(AlgebraicModel):
    """Apply arbitrary function to predictions, or use function as a standalone model."""

    def __init__(self, model_or_func: Union[AlgebraicModel, Callable],
                 func: Optional[Callable[[Dict[str, float]], Dict[str, float]]] = None):
        if func is None:
            # model_or_func is a standalone prediction function
            self.model = None
            self.func = model_or_func
        else:
            # model_or_func is a base model, func is a transformation
            self.model = model_or_func
            self.func = func

    def predict(self, context: List[str], top_k: int = 50) -> Dict[str, float]:
        if self.model is None:
            # Standalone function model
            predictions = self.func(context, top_k)
        else:
            # Transform predictions from base model
            predictions = self.model.predict(context, top_k * 2)
            predictions = self.func(predictions)

        # Ensure valid probabilities
        total = sum(predictions.values()) if predictions else 0
        if total > 0:
...
class AttentionOperator(ModelOperator):
    ...
    def __init__(self, temperature: float = 1.0, key_fn: Optional[Callable[[List[str]], np.ndarray]] = None):
        """
        Initialize attention operator.

        Args:
            temperature: Temperature for softmax attention weights
            key_fn: Optional function to compute keys from context
        """
        self.temperature = temperature
        self.key_fn = key_fn or self._default_key_fn

    def _default_key_fn(self, context: List[str]) -> np.ndarray:
        """Default key function based on context length."""
        return np.array([len(context)])

    def apply(self, models: List[AlgebraicModel],
              context: List[str]) -> Dict[str, float]:
        """Apply attention-based combination."""
        if not models:
            raise ValueError("AttentionOperator requires at least one model")
    ...
    def _compute_attention(self, models: List[AlgebraicModel],
        ...
        # Simplified: use random scores (in practice, would compute from models)
        scores = np.random.randn(num_models)
        # Apply temperature and softmax
        weights = np.exp(scores / self.temperature) / np.sum(np.exp(scores / self.temperature))
...
class GatingOperator(ModelOperator):
    ...
    def __init__(self, gate_threshold: float = 0.5, gate_fn: Optional[Callable[[List[str]], List[float]]] = None):
        """
        Initialize gating operator.

        Args:
            gate_threshold: Threshold for gating (gates below this are set to 0)
            gate_fn: Optional function to compute gates from context
        """
        self.gate_threshold = gate_threshold
        self.gate_fn = gate_fn or self._default_gate_fn

    def _default_gate_fn(self, context: List[str]) -> List[float]:
        """Default gate function - equal weights if above threshold."""
        # Simple heuristic based on context length
        if len(context) >= 3:
            return [0.6, 0.4]  # Favor first model for longer contexts
        else:
            return [0.4, 0.6]  # Favor second model for shorter contexts
    ...
    def apply(self, models: List[AlgebraicModel],
        ...
        # Ensure we have enough gates
        if len(gates) < len(models):
            gates = gates + [0.0] * (len(models) - len(gates))

        combined = defaultdict(float)
        for model, gate in zip(models, gates):
            # Apply threshold
            effective_gate = gate if gate >= self.gate_threshold else 0.0
            if effective_gate > 0:
                predictions = model.predict(context, 100)
                for token, prob in predictions.items():
                    combined[token] += effective_gate * prob
...
class ModelBuilder:
    ...
    def apply_transform(self, transform: ContextTransform) -> 'ModelBuilder':
        """Apply context transformation (alias for transform)."""
...
def create_ensemble(models: List[AlgebraicModel],
    ...
    Methods:
        - 'average': Simple average (or weighted average if weights provided)
        - 'mixture': Explicit mixture with weights
        - 'max': Take maximum probability
        - 'min': Take minimum (conservative)
        - 'product': Geometric mean
        - 'voting': Majority voting (max probability)
    """
    if method in ('average', 'mixture'):
        if weights is None:
            weights = [1.0 / len(models)] * len(models)
        return MixtureModel(models, weights)
    elif method in ('max', 'voting'):
        # Voting is implemented as max probability
